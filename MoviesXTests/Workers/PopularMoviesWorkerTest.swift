...
class PopularMoviesWorkerTest: XCTestCase {
    ...
    override func setUp() {
        ...
        worker = PopularMoviesWorker(session: session)
    }
    ...
    func testSuccessTopRatedMovies() {
        ...
        let url = URL(string: worker.backendUrlWorkerObj.getPopularMoviesURL())
        ...
        worker.getPopularMovies(page: 1) { (success, moviesResponse) in
