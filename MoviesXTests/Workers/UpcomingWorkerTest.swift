...
class UpcomingWorkerTest: XCTestCase {
    ...
    var worker: UpcomingWorker!

    override func setUp() {
        super.setUp()
        
        session = MockUrlSession()
        worker = UpcomingWorker(session: session)
    }
    ...
    func testSuccessUpcomingMovies() {

        let data = Data(self.mockSuccessMoviesResponse.utf8)
        session.nextData = data
        let url = URL(string: worker.backendUrlWorkerObj.getUpcomingMoviesURL())
        ...
        worker.getUpcomingMovies(page: 1) { (success, moviesResponse) in
